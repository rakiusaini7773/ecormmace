...
const AddBlogForm = () => {
  ...
  const [editorKey, setEditorKey] = useState(0); // used to force RTE reset
    ...
    try {
      ...
      const processed = response.map((blog) => ({
        ...
        image: blog.image?.startsWith('http')
          ? blog.image
          : `${process.env.REACT_APP_BASE_URL}/uploads/${blog.image}`,
      }));
      setBlogs(processed);
    } catch (error) {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    formData.append('description', blogContent); // HTML from editor
  ...
  const columns = [
    ...
    { Header: 'Category', accessor: 'category' },
    {
      ...
      Cell: ({ row }) => (
        <span>
          {stripHtmlTags(row.original.description).substring(0, 100)}...
        </span>
      ),
    },
    {
          ...
          {({ setFieldValue }) => (
            <Form className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm mb-1">Blog Content</label>
                <RichTextEditor key={editorKey} value={blogContent} onChange={setBlogContent} />
              </div>
