  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Build coupon map: productId → [couponTypes]
        const couponMap = {};
        ...
        // Add applicableCoupons array to each product
                    ...
                    {item.applicableCoupons.length > 0 ? (
                      <div className="text-gray-600 text-sm flex flex-col gap-1">
                        {item.applicableCoupons.map((coupon, idx) => (
                          <p key={idx} className="flex items-center gap-1" style={{ fontSize: offerFontSize }}>
